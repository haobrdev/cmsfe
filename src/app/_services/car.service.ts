...
Injectable();
export class CarService extends Subject<DataStateChangeEventArgs> {
  ...
  public getlist(state: any,extraParams?: Array<ExtraParams>): void {
    const url = API.car  + API.list;
    this.getall(state, url, extraParams).subscribe((x) => super.next(x));
  }

  // Service  delete 
  delete = (body: any) => {
    const url_request = this.globals.apiURL + API.car  + API.delete;
  ...
  getprimaryKey = (paramId: any) => {
    ...
    const url_request = this.globals.apiURL + API.car + paramId;
  ...
  create = (body: any) => {
    const url_request = this.globals.apiURL + API.car + API.create ;
    const options = this.globals.getCommonOptionsWithAuth(url_request, body);
    return this.http.post(url_request, body, options).pipe(
      map((response: any) => {
        response = JSON.parse(response._body);
  ...
  update = (body: any) => {
    const url_request = this.globals.apiURL + API.car + API.update ;
    const options = this.globals.getCommonOptionsWithAuth(url_request, body);
    return this.http.post(url_request, body, options).pipe(
      map((response: any) => {
        response = JSON.parse(response._body);
