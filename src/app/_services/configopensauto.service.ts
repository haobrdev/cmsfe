...
Injectable();
export class ConfigOpensAutoService extends Subject<DataStateChangeEventArgs> {
  ...
  public getlist(state: any,extraParams?: Array<ExtraParams>): void {
    const url = API.configopensauto  + API.list;
    this.getall(state, url, extraParams).subscribe((x) => super.next(x));
  }

  // Service  delete 
  delete = (body: any) => {
    const url_request = this.globals.apiURL + API.configopensauto  + API.delete;
  ...
  getprimaryKey = (paramId: any) => {
    ...
    const url_request = this.globals.apiURL + API.configopensauto + paramId;
  ...
  create = (body: any) => {
    const url_request = this.globals.apiURL + API.configopensauto + API.create ;
    const options = this.globals.getCommonOptionsWithAuth(url_request, body);
    return this.http.post(url_request, body, options).pipe(
      map((response: any) => {
        response = JSON.parse(response._body);
  ...
  update = (body: any) => {
    const url_request = this.globals.apiURL + API.configopensauto + API.update ;
    const options = this.globals.getCommonOptionsWithAuth(url_request, body);
    return this.http.post(url_request, body, options).pipe(
      map((response: any) => {
        response = JSON.parse(response._body);
