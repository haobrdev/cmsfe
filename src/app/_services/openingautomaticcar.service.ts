...
Injectable();
export class OpeningautomaticcarService extends Subject<DataStateChangeEventArgs> {
  ...
  public getlist(state: any,extraParams?: Array<ExtraParams>): void {
    const url = API.openingautomaticcar  + API.list;
    this.getall(state, url, extraParams).subscribe((x) => super.next(x));
  }


  update = (body: any) => {
    const url_request = this.globals.apiURL + API.openingautomaticcar + API.update ;
  ...
  changeacctive = (body: any) => {
    const url_request = this.globals.apiURL + API.openingautomaticbylincesetype + API.changeacctive ;
    const options = this.globals.getCommonOptionsWithAuth(url_request, body);
    return this.http.post(url_request, body, options).pipe(
      map((response: any) => {
        response = JSON.parse(response._body);
