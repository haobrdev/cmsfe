...
import { AccompanyingvehiclemanageService } from "src/app/_services/accompanyingvehiclemanage.service";
...
import { AccompanyingVehicleManage } from "src/app/_models/business/AccompanyingVehicleManage";
import { Notification } from "src/app/common/notification";

@Component({
  selector: "accompanying-vehicle-manage-modals",
  styleUrls: ["./accompanying-vehicle-manage-modals.component.scss"],
  templateUrl: "./accompanying-vehicle-manage-modals.component.html",
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class AccompanyingvehiclemanageModalComponent implements OnInit, OnDestroy
{
  @Input() id: string;
  private element: any;
  flagState = "";
  ...
  isShow = false;
  editForm: FormGroup;
  model: AccompanyingVehicleManage = new AccompanyingVehicleManage();
  listAccompanyingVehicleManage: Array<AccompanyingVehicleManage>;
  public modelAdd: any;
  ...
  public lstAccompanyingVehicleManage: any = [];
  ...
  constructor(
    ...
    private _accompanyingvehiclemanageService: AccompanyingvehiclemanageService,
    ...
    this.data = _accompanyingvehiclemanageService;
  ...
  ngOnInit(): void {
    // Build toolbar
    this.getVehicletypeComboBoxData();
    ...
    setTimeout(() => {
      ...
      if (this.dataFromParent[0].listaccompanyingvehicleManage != null) {
        this.data_template = this.dataFromParent[0].listaccompanyingvehicleManage;
        this.data_delete =[];
  ...
  clickRecord = (data, status) => {
    ...
    if (this.isDisable) 
    {
      ...
      if (data && status === "delete") {
        ...
        this.modalService.open("confirm-delete-v-one-modal");
      }
    }
  };
  ...
  confirmDeleteOne = (status): void => {
    if (status === "cancel") {
      this.modalService.close("confirm-delete-v-one-modal");
    } else {
      // gọi API xóa ở đây
      if (this.modelDelete) {
        if (!this.modelDelete.isNew) this.data_delete.push(this.modelDelete);
        
        this.listAccompanyingVehicleManage = [];
        let data = this.data_template.filter(x => x.id != this.modelDelete.id);
        data.forEach(item => {
          this.listAccompanyingVehicleManage.push(item);
        });
        this.convertModelToAny(this.listAccompanyingVehicleManage);
      }
      this.modalService.close("confirm-delete-v-one-modal");
    }
  };
    ...
    if (result != null)
      {
        ...
        this.avatar = this.model.photo_st_img == "" ? "/assets/images/addPicture_Vehicle.png": this.model.photo_st_img ;
        localStorage.setItem('avatar', this.avatar);

        //Ảnh 2
        this.avatar_back = this.model.photo_rd_img =="" ? "/assets/images/addPicture_Vehicle.png": this.model.photo_rd_img ;
        localStorage.setItem('avatar_back', this.avatar_back);
  ...
  getListData = (): void => {
    ...
    const url = "/accompanyingvehicleManage/list";
    this._accompanyingvehiclemanageService
      .getall(state, "/accompanyingvehicleManage/list", extraParams)
      .subscribe((res) => {
        this.data = res;
      });
  };
  ...
  getVehicletypeComboBoxData = () => {
    async.parallel([(cb) => {
      this._coreService
      .Get("/dropdown/otherListByCode/car_type")
        .subscribe((res) => {
          if (res.code == "200") {
            this.lstVehicleType = res.data;
            setTimeout(() => {
              if(this.lstVehicleType[1] && this.lstVehicleType[1] !== undefined && this.flagState !== "view")
              {
                this.model.vehicle_type = this.lstVehicleType[1].id;
              }
            }, 200);
            return cb();
          }
  ...
  createModel() {
    this.model = new AccompanyingVehicleManage();
    this.model.id = this._accompanyingvehiclemanageService.GetNewGuid();
    this.model.license_no = null;
    this.model.vehicle_type_name ="";
    this.model.vehicle_color="";
    this.model.photo_st_img = "";
    this.model.photo_rd_img = "";
    if(this.flagState !== "view")
    {
      this.model.vehicle_type = this.lstVehicleType[1].id;
      this.model.time_in = this.globals.convertDateStringFull(new Date());
  ...
  prepareModelBeforeSave = () => {
    ...
    objAPI.photo_st_img = this.avatar;
    objAPI.photo_rd_img = this.avatar_back;
  ...
  convertItemModel() {
    let item = this.lstVehicleType.filter((x) => x.id == this.model.vehicle_type)[0];
    this.model.vehicle_type_name = item != null ? item.name : "";
  ...
  saveData() {
    if (!this.editForm.valid) {
      ...
      if (this.flagStateItem === "new") {
        ...
        this.listAccompanyingVehicleManage.push(this.model)
  ...
  convertModelToAny(lst: Array<AccompanyingVehicleManage>)
  {
    const newArray = lst
    .map(modal => ({
      id: modal.id, 
      infoinout_id: modal.infoinout_id, 
      license_no: modal.license_no, 
      vehicle_type: modal.vehicle_type, 
      vehicle_type_name: modal.vehicle_type_name, 
      vehicle_color: modal.vehicle_color, 
      photo_st_img: modal.photo_st_img, 
      photo_rd_img: modal.photo_rd_img, 
