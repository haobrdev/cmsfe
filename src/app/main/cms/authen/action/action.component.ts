...
export class ActionComponent implements OnInit {
  ...
  getComboBoxData = () => {
    async.parallel([(cb) => {
      this._coreService.Get("/dropdown/function").subscribe((res) => {
        if (res && res.code && res.code === "200") {
          this.lstFunctions = res.data;
        }
      })
    }])
  }
  ...
  getListData = (): void => {
    ...
    if (this.search.function_id) {
      extraParams.push({
        field: "function_id",
        value: this.search.function_id
      });
    }

    this._coreService.execute(state, "/action/list", extraParams);
  };
  ...
  buildToolbar = () => {
    ...
    this.toolbar = this.globals.buildToolbar("cms_authen_action", toolbarList);


    let toolbarList2 = [];
    toolbarList2 = [ToolbarItem.EDIT, ToolbarItem.DELETE];
    this.toolbar2 = this.globals.buildToolbar("cms_authen_action", toolbarList2);
  };
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/cms/authen/action/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          this.router.navigate(["/cms/authen/action", paramAdd]);
        } else {
  ...
  confirmDeleteOne = (status): void => {
    if (status === "cancel") {
      ...
      if (this.modelDelete) {
        this._coreService.Post("/action/delete", {
  ...
  changeStatus = (data) => {
    setTimeout(() => {
      ...
      this._coreService.Post("/action/changeStatus", {
        id: data.id,
