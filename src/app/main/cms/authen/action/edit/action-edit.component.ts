...
import { Action } from "src/app/_models/authen/Action";
...
export class ActionEditComponent implements OnInit {
      ...
      [
        ...
        (cb) => {
          if (this.flagState != "new") {
            this._coreService
              .Get("/action/" + this.paramId)
              .subscribe((res) => {
                if (res.code == "200") {
                  this.model = res.data;
                  if (this.model.status == 1) {
                    this.model.status_id = true;
                  } else {
                    this.model.status_id = false;
                  }
      ...
      this._coreService.Post(url, modelRequest).subscribe(
        (res) => {
          if (res && res.code == "200") {
            ...
            this.router.navigate(["/cms/authen/action"]);
          } else {
  ...
  prepareModelBeforeSave = () => {
    ...
    objAPI.status = this.model.status;
    objAPI.is_default = true;
    objAPI.web_type = "ADMIN";
