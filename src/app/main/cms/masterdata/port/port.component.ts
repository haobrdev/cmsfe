...
export class PortComponent implements OnInit {
  ...
  getListData = (): void => {
    ...
    if (this.search.port_name) {
      extraParams.push({
        field: "port_name",
        value: this.search.port_name.trim(),
      });
    }

    this._coreService.execute(state, "/port/list", extraParams);
  };
  ...
  buildToolbar = () => {
    ...
    this.toolbar = this.globals.buildToolbar("cms_masterdata_port", toolbarList);

    
    let toolbarList2 = [];
    toolbarList2 = [ToolbarItem.EDIT, ToolbarItem.DELETE , ToolbarItem.VIEW];
    this.toolbar2 = this.globals.buildToolbar("cms_masterdata_port", toolbarList2);
  };
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/cms/masterdata/port/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          this.router.navigate(["/cms/masterdata/port", paramAdd]);
        } else {
  ...
  confirmDeleteOne = (status): void => {
    if (status === "cancel") {
      ...
      // Call API Delete
      if (this.modelDelete) {
        this._coreService.Post("/port/delete", {
