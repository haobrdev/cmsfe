...
export class TitleComponent implements OnInit {
  ...
  getComboBoxData = () => {
    async.parallel([(cb) => {
      this._coreService
      .Get("/dropdown/otherListByCode/status")
      .subscribe((res) => {
        if (res.code == "200") {
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(state, "/title/list", extraParams);
  };
  ...
  buildToolbar = () => {
    ...
    this.toolbar = this.globals.buildToolbar("cms_masterdata_title", toolbarList);

    
    let toolbarList2 = [];
    toolbarList2 = [ToolbarItem.EDIT, ToolbarItem.DELETE];
    this.toolbar2 = this.globals.buildToolbar("cms_masterdata_title", toolbarList2);
  };
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/cms/masterdata/title/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          this.router.navigate(["/cms/masterdata/title", paramAdd]);
        } else {
  ...
  confirmDeleteOne = (status): void => {
    if (status === "cancel") {
      ...
      if (this.modelDelete) {
        this._coreService.Post("/title/delete", {
