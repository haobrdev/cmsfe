...
export class WardComponent implements OnInit {
  ...
  getListData = (): void => {
    ...
    if (this.search.province_id) {
      extraParams.push({
        field: "province_id",
        value: this.search.province_id
      });
    }
    ...
    if (this.search.district_id) {
      extraParams.push({
        field: "district_id",
        value: this.search.district_id
      });
    }

    this._coreService.execute(state, "/province/list", extraParams);
  };
  ...
  buildToolbar = () => {
    ...
    this.toolbar = this.globals.buildToolbar("cms_masterdata_ward", toolbarList);

    
    let toolbarList2 = [];
    toolbarList2 = [ToolbarItem.EDIT, ToolbarItem.DELETE];
    this.toolbar2 = this.globals.buildToolbar("cms_masterdata_ward", toolbarList2);
  };
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/cms/masterdata/ward/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          this.router.navigate(["/cms/masterdata/ward", paramAdd]);
        } else {
  ...
  changeProvince = (e: any) => {
    if (e.isInteracted) {
      ...
      setTimeout(() => {
        if (this.search.province_id) {
          this._coreService.Get("/dropdown/provinceBy/" + this.search.province_id).subscribe((data) => {
  ...
  confirmDeleteOne = (status): void => {
    if (status === "cancel") {
      ...
      if (this.modelDelete) {
        this._coreService.Post("/province/delete", {
