import {
  ...
  @Component({
    selector: "cms-cardusedoffice",
    templateUrl: "./cardusedoffice.component.html",
    styleUrls: ["./cardusedoffice.component.scss"],
    providers: [FilterService, VirtualScrollService],
    encapsulation: ViewEncapsulation.None,
  })
  export class CardusedofficeComponent implements OnInit {
  // Varriable Language
  languages: any;
  ...
  public search = {card_type:null ,department: "", status: null};

  // List dropdown list
  public lstTypeCard = [];
  public lstOrganization = [];
  public lstStatus = [];
  ...
  getListData = (): void => {
    ...
    if (this.search.card_type) {
        extraParams.push({
          field: "card_type",
          value: this.search.card_type,
        });
    }
    ...
    this._coreService.execute(state, "/cardusedoffice/list", extraParams);
  };
  ...
  buildToolbar = () => {
    ...
    this.toolbar = this.globals.buildToolbar("cms_report_cardusedoffice", toolbarList);
    let toolbarList2 = [];
    toolbarList2 = [ToolbarItem.EDIT, ToolbarItem.DELETE];
    this.toolbar2 = this.globals.buildToolbar("cms_report_cardusedoffice", toolbarList2);
  };
  ...
  exportData = () => {
      let file_name =  "BÁO CÁO DANH SÁCH THẺ DÙNG TRONG CƠ QUAN_" + moment().format('YYYYMMDDHHmmss');
      this._coreService.PostExport("/cardusedoffice/export", {
        page_no: this.pageIndex,
        page_size: this.state.take,
        card_type: this.search.card_type != null ? this.search.card_type : null,
        department: this.search.department != "" ? this.search.department : null,
        status: this.search.status != null ? this.search.status : null
      }).subscribe((data) => {
